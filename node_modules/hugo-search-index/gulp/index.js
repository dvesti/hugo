...
const front_matter_parser_1 = require("./front_matter_parser");
const exportFile = './public/search_index.gz';
const hugoConfig = toml.parse(fs.readFileSync('./config.toml').toString());
...
const buildDocument = () => new stream_1.Transform({
    objectMode: true,
    transform(obj, encoding, callback) {
        ...
        try {
            ...
            const { frontMatter, contents } = front_matter_parser_1.default(data);
            if (frontMatter) {
 ...
 */
const invertDocumentVector = () => new stream_1.Transform({
    objectMode: true,
    transform(obj, encoding, callback) {
        const doc = obj;
...
function exportIndex(index, file, c, done) {
...
function moveZippedFile(file, done) {
...
function buildSearchIndex(gulp, c, done) {
        ...
        fs.mkdir('./public', (mdErr) => {
            ...
            gulp.src('./content/**/*.md')
                ...
                index.countDocs((countErr, count) => {
                    ...
                    if (c) {
                        c.log(chalk.gray(`loaded ${count} documents in search index.  Exporting to ${exportFile}`));
                    }
                    exportIndex(index, exportFile, c, (exportErr) => {
                        ...
                        if (c) {
                            c.log(chalk.gray('closing temporary search index...'));
                        }
                        index.close((closeErr) => {
                            ...
                            if (c) {
                                c.log(chalk.gray('removing temporary search index files...'));
                            }
                            fs.remove('si', (rmErr) => {
                                if (rmErr) {
                                    done(rmErr);
...
const gulp3 = (gulp, c) => {
    ...
    gulp.task('hugo-search-index/copy-javascript', () => gulp.src(['node_modules/search-index/dist/search-index.min.js', 'node_modules/hugo-search-index/dist/search.min.js'])
        .pipe(gulp.dest('./public/js')));
    gulp.task('hugo-search-index/build-search-index', (done) => {
        buildSearchIndex(gulp, c, done);
    });
};
const gulp4 = (gulp, c) => {
    gulp.task('hugo-search-index/copy-javascript', () => gulp.src(['node_modules/search-index/dist/search-index.min.js', 'node_modules/hugo-search-index/dist/search.min.js'])
    ...
    gulp.task('hugo-search-index', gulp.parallel('hugo-search-index/copy-javascript', 'hugo-search-index/build-search-index'));
};
